...
"""
    ...
    def _nm_score_candidates(
        ...
        for i, candidate in enumerate(scored_candidates[:10]):  # Show top 10
            self.log(f"  #{i+1}: ID {candidate.id}, score {candidate.total_score:.1f}", "info")
    ...
    def _nm_generate_candidates(
        ...
        if use_ocr:
            if self.use_paddleocr and self.init_paddleocr():
                self.log("OCR: using PaddleOCR (sv/Latin multilingual) for ROI.", "info")
                try:
                    from agent.ocr import OCRResult
                    paddle_matches = self._nm_run_paddleocr_roi(img, roi)
                    if paddle_matches:
                        ocr_result = OCRResult(
                            matches=paddle_matches,
                            raw_text=" ".join([m.text for m in paddle_matches]),
                        )
                        self.log(f"PaddleOCR in ROI produced {len(ocr_result.matches)} text matches.", "info")
                    else:
                        self.log("PaddleOCR in ROI found no matches – falling back to Tesseract.", "info")
                except Exception as e:
                    self.log(f"PaddleOCR failed in ROI: {e} – falling back to Tesseract.", "error")

            if ocr_result is None:
                ocr_engine = get_ocr_engine()
                roi_crop = img.crop((roi.left, roi.top, roi.right, roi.bottom))

                # Try OCR with standard processing
        ...
        if ocr_result.matches:
            ocr_result.matches = self._nm_refine_ocr_matches(img, ocr_result.matches)
        ...
        if use_color:
            ...
            color_regions = self._nm_find_color_regions(
                img,
                roi,
                target_color,
                accent,
        ...
        for match in ocr_result.matches:
            ...
            click_x, click_y = self._nm_text_center_in_bbox(img, (left, top, right, bottom))
            color = self._nm_compute_region_color(img, (left, top, right, bottom))

            text_sim = self._nm_text_similarity(planner.text_intent, text)
            ...
            if 0.8 <= aspect <= 8.0:  # Much wider range
                shape_score = 1.0
            elif 0.5 <= aspect <= 12.0:  # Still acceptable
                shape_score = 0.7
            else:
                shape_score = 0.5  # Even very tall/wide gets some score instead of 0.4
            ...
            location_score = max(0.0, 1.0 - dist_norm)  # closer to center → higher
        ...
        if color_candidates_skipped > 0:
            self.log(f"Skipped {color_candidates_skipped} tiny color-mask regions (area<{color_noise_area_min}px).", "info")
