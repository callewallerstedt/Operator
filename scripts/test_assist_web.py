...
def _request(method: str, url: str, token: str, body: bytes | None = None, content_type: str | None = None):
...
def main() -> int:
    ...
    parser.add_argument("--image", dest="image_path", default="", help="Local image path to upload")
    ...
    if not args.channel or not args.token or not args.assist_url:
        print("Missing DISCORD_TEST_CHANNEL_ID, DISCORD_BOT_TOKEN, or ASSIST_WEB_URL", file=sys.stderr)
        return 2
    ...
    if not attachment_url:
        if not args.image_path:
            ...
            return 2
        resp = _send_with_file(args.token, args.channel, f"Assist test image (session {session_id}).", args.image_path)
